...
int init_router_discovery_relay(const struct relayd_config *relayd_config) {
  ...
  if (config->enable_router_discovery_server) {
    ...
    int zero = 0;
    setsockopt(router_discovery_event.socket, IPPROTO_IPV6, IPV6_MULTICAST_LOOP,
               &zero, sizeof(zero));
    ...
    setsockopt(router_discovery_event.socket, IPPROTO_IPV6, IPV6_ADD_MEMBERSHIP,
               &an, sizeof(an));
  }
...
static int open_icmpv6_socket(struct icmp6_filter *filt,
  ...
  for (size_t i = 0; i < config->slavecount; ++i) {
    ...
    setsockopt(sock, IPPROTO_IPV6, IPV6_ADD_MEMBERSHIP, slave_mreq,
               sizeof(*slave_mreq));
  }
...
static bool match_route(const struct relayd_ipaddr *n,
  ...
  while (fgets(line, sizeof(line), fp_route)) {
    ...
    if (sscanf(line,
      ...
      found_default = true;
                   ...
                   " %hhx %*s "
                   "%*s 00000000000000000000000000000000 %*s %*s %*s %" SCNx32
                   " lo",
                   &p.addr.s6_addr32[0], &p.addr.s6_addr32[1], &p.prefix,
                   &rflags) &&
               p.prefix > 0 && (rflags & RTF_NONEXTHOP) &&
               (rflags & RTF_REJECT)) {
...
static void send_router_advert(struct relayd_event *event) {
  ...
  if (!in_shutdown) {
    ipcnt = relayd_get_interface_addresses(iface->ifindex, addrs,
                                           ARRAY_SIZE(addrs));
  ...
  relayd_forward_packet(router_discovery_event.socket, &all_nodes, iov, 4,
                        iface);
  ...
  val.it_value.tv_sec =
      (labs(val.it_value.tv_sec) % (MaxRtrAdvInterval - MinRtrAdvInterval)) +
      MinRtrAdvInterval;
  ...
  for (size_t i = 0; i < config->slavecount; ++i) {
    ...
    if (config->always_rewrite_dns && dns_ptr && dns_count > 0) {
      ...
      if (!IN6_IS_ADDR_UNSPECIFIED(&config->dnsaddr)) {
        ...
        if (relayd_get_interface_addresses(config->slaves[i].ifindex, &addr,
                                           1) < 1)
