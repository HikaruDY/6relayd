...
static int open_interface(struct relayd_interface *iface, const char *ifname,
                          bool external);
...
int main(int argc, char *const argv[]) {
  ...
  while ((c = getopt(argc, argv, "ASR:D:Nsucn::l:a:rt:m:oi:p:dvh:P:")) != -1) {
    ...
    case 't':
      config.static_ndp =
          realloc(config.static_ndp, sizeof(char *) * ++config.static_ndp_len);
...
static void set_stop(_unused int signal) { do_stop = true; }
...
static void relayd_receive_packets(struct relayd_event *event) {
  ...
  while (true) {
    ...
    for (struct cmsghdr *ch = CMSG_FIRSTHDR(&msg); ch != NULL && destiface == 0;
    ...
    syslog(LOG_NOTICE, "--");
    syslog(LOG_NOTICE, "Received %li Bytes from %s%%%s", (long)len, ipbuf,
           (iface) ? iface->ifname : "netlink");

    event->handle_dgram(&addr, data_buf, len, iface);
  }
}

void relayd_urandom(void *data, size_t len) { read(urandom_fd, data, len); }
