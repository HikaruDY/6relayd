...
int dhcpv6_init_ia(const struct relayd_config *relayd_config,
                   int dhcpv6_socket) {
...
static int send_reconf(struct relayd_interface *iface,
...
static void write_statefile(void) {
  if (config->dhcpv6_statefile) {
    ...
    int fd =
        open(config->dhcpv6_statefile, O_CREAT | O_WRONLY | O_CLOEXEC, 0644);
    if (fd < 0) {
      return;
    }
    lockf(fd, F_LOCK, 0);
...
static void apply_lease(struct relayd_interface *iface, struct assignment *a,
                        bool add) {
  if (a->length > 64)
    return;
...
static bool assign_pd(struct relayd_interface *iface,
                      struct assignment *assign) {
  struct assignment *c;
  ...
  if (assign->assigned) {
    list_for_each_entry(c, &iface->pd_assignments, head) {
      ...
      if (assign->assigned >= current &&
          assign->assigned + asize < c->assigned) {
...
static bool assign_na(struct relayd_interface *iface,
                      struct assignment *assign) {
  if (iface->pd_addr_len < 1)
    return false;
...
static void update(struct relayd_interface *iface) {
  ...
  struct assignment *border =
      list_last_entry(&iface->pd_assignments, struct assignment, head);
  ...
  if (change) {
    struct assignment *c;
    list_for_each_entry(c, &iface->pd_assignments, head) if (c != border)
  ...
  if (change) { // Addresses / prefixes have changed
    ...
    list_for_each_entry_safe(c, d, &iface->pd_assignments, head) {
      ...
      if (c->accept_reconf && c->reconf_cnt == 0) {
        ...
        struct assignment *a;
        list_for_each_entry(
            a, &iface->pd_assignments,
            head) if (a != c && a->clid_len == c->clid_len &&
      ...
      dhcpv6_for_each_option((uint8_t *)&ia[1], end, otype, olen, odata) {
        ...
        if (!found) {
          if (otype == DHCPV6_OPT_IA_PREFIX) {
            ...
            struct dhcpv6_ia_addr inv = {.type = htons(DHCPV6_OPT_IA_ADDR),
                                         .len = htons(sizeof(inv) - 4),
                                         .addr = n->addr,
                                         .preferred = 0,
                                         .valid = 0};
...
size_t dhcpv6_handle_ia(uint8_t *buf, size_t buflen,
                        struct relayd_interface *iface,
                        const struct sockaddr_in6 *addr, const void *data,
                        const uint8_t *end) {
  time_t now = monotonic_time();
  ...
  dhcpv6_for_each_option(start, end, otype, olen, odata) {
    if (otype == DHCPV6_OPT_CLIENTID) {
      ...
      if (dn_expand(&fqdn_buf[1], &fqdn_buf[olen], &fqdn_buf[1], hostname,
                    sizeof(hostname)) > 0)
  ...
  dhcpv6_for_each_option(start, end, otype, olen, odata) {
    ...
    if (is_pd) {
      ...
      dhcpv6_for_each_option(&ia[1], odata + olen, stype, slen, sdata) {
        if (stype == DHCPV6_OPT_IA_PREFIX &&
            slen >= sizeof(struct dhcpv6_ia_prefix) - 4) {
    ...
    if (hdr->msg_type == DHCPV6_MSG_SOLICIT ||
      ...
      if (!assigned || iface->pd_addr_len == 0) { // Set error status
        status =
            (is_pd) ? DHCPV6_STATUS_NOPREFIXAVAIL : DHCPV6_STATUS_NOADDRSAVAIL;
      ...
      if (!a && hdr->msg_type != DHCPV6_MSG_REBIND) {
        ...
        ia_response_len =
            append_reply(buf, buflen, status, ia, a, iface, false);
      } else if (hdr->msg_type == DHCPV6_MSG_RENEW ||
                 hdr->msg_type == DHCPV6_MSG_REBIND) {
        ia_response_len =
            append_reply(buf, buflen, status, ia, a, iface, false);
