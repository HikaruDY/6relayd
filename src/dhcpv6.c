...
static void relay_client_request(struct sockaddr_in6 *source, const void *data,
                                 size_t len, struct relayd_interface *iface);
...
int init_dhcpv6_relay(const struct relayd_config *relayd_config) {
  ...
  if ((dhcpv6_event.socket = create_socket(DHCPV6_SERVER_PORT)) < 0) {
    syslog(LOG_ERR, "Failed to open DHCPv6 server socket: %s", strerror(errno));
    return -1;
  }
  ...
  for (size_t i = 0; i < config->slavecount; ++i) {
    ...
    setsockopt(dhcpv6_event.socket, IPPROTO_IPV6, IPV6_ADD_MEMBERSHIP, &mreq,
               sizeof(mreq));

    if (config->enable_dhcpv6_server) {
      mreq2.ipv6mr_interface = config->slaves[i].ifindex;
      setsockopt(dhcpv6_event.socket, IPPROTO_IPV6, IPV6_ADD_MEMBERSHIP, &mreq2,
                 sizeof(mreq2));
    }
  }
...
static int create_socket(uint16_t port) {
  ...
  struct sockaddr_in6 bind_addr = {AF_INET6, htons(port), 0, IN6ADDR_ANY_INIT,
                                   0};
...
static void handle_nested_message(uint8_t *data, size_t len, uint8_t **opts,
                                  uint8_t **end, struct iovec iov[5]) {
  ...
  dhcpv6_for_each_option(hdr->options, data + len, otype, olen, odata) {
    if (otype == DHCPV6_OPT_RELAY_MSG) {
      ...
      iov[5].iov_len =
          (((uint8_t *)iov[0].iov_base) + iov[0].iov_len) - (odata + olen);
...
static void handle_client_request(void *addr, void *data, size_t len,
  ...
  } dest = {.msg_type = DHCPV6_MSG_REPLY,
  ...
  uint8_t pdbuf[512];
  struct iovec iov[] = {
      {NULL, 0},     {&dest, (uint8_t *)&dest.clientid_type - (uint8_t *)&dest},
      {&dnsaddr, 0}, {&domain, domain_len},
      {pdbuf, 0},    {NULL, 0}};
  ...
  if (opts[-4] == DHCPV6_MSG_ADVERTISE || opts[-4] == DHCPV6_MSG_REPLY ||
      opts[-4] == DHCPV6_MSG_RELAY_REPL)
    return;
  ...
  if (opts[-4] != DHCPV6_MSG_INFORMATION_REQUEST) {
    iov[4].iov_len = dhcpv6_handle_ia(pdbuf, sizeof(pdbuf), iface, addr,
                                      &opts[-4], opts_end);
...
static void relay_server_response(uint8_t *data, size_t len) {
  ...
  int32_t ifaceidx = 0;
  struct sockaddr_in6 target = {AF_INET6, htons(DHCPV6_CLIENT_PORT), 0,
                                IN6ADDR_ANY_INIT, 0};
  ...
  dhcpv6_for_each_option(h->options, end, otype, olen, odata) {
    if (otype == DHCPV6_OPT_INTERFACE_ID && olen == sizeof(ifaceidx)) {
...
static void relay_client_request(struct sockaddr_in6 *source, const void *data,
  ...
  if (h->msg_type == DHCPV6_MSG_RELAY_REPL ||
      ...
      h->msg_type == DHCPV6_MSG_REPLY || h->msg_type == DHCPV6_MSG_ADVERTISE)
  ...
  if (relayd_get_interface_addresses(iface->ifindex, &ip, 1) < 1) {
    ...
    if (relayd_get_interface_addresses(config->master.ifindex, &ip, 1) < 1)
      return; // Could not obtain a suitable address
  }
  memcpy(&hdr.link_address, &ip.addr, sizeof(hdr.link_address));

  struct sockaddr_in6 dhcpv6_servers = {AF_INET6, htons(DHCPV6_SERVER_PORT), 0,
                                        ALL_DHCPV6_SERVERS, 0};
